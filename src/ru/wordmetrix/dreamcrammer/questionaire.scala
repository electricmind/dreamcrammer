...
import android.view.{ Menu, MenuItem, MenuInflater, View, LayoutInflater, ViewGroup }
import android.widget.{ ToggleButton, Button, ImageView, TextView, PopupMenu, ListView }
import android.graphics.{ BitmapFactory, Bitmap }
import android.support.v4.widget.{ SlidingPaneLayout, DrawerLayout }
...
class Questionaire extends DreamCrammerBase with MenuQuestionaire {
    ...
    def doPostpone() = queue.exists(x => { publish(x.postpone()); true })

    def doReload() = queue.exists(x => { publish(x.reload()); true })

    def doDelete() = queue.exists(x => { publish(x.disable(true)); true })

    override def doPlayer = {
        startActivity(new Intent(this, classOf[Player]) {
            putExtra("word_ids", db.query(
                """
