...
import android.widget.{ArrayAdapter, Button, CompoundButton, GridView, ListView, PopupMenu, TextView, ToggleButton}
import ru.wordmetrix._
import ru.wordmetrix.dreamcrammer.db._
import scala.concurrent.ExecutionContext.Implicits.global

class PhraseListAdapter(context: DreamCrammerBase, resource: Int, items: List[Phrase])
  extends ArrayAdapter(context, resource, items.foldLeft(new java.util.ArrayList[Phrase]())((array, x) => {
    array.add(x);
    array
  })) {
  ...
  private def speakButton(view: View, phrase: Phrase) = {
    Option(view.findViewById(R.id.phrase_track)) foreach {
      case button: Button =>
        button.setOnClickListener(new View.OnClickListener() {
          override def onClick(view: View): Unit =
            context.textToSpeach.foreach {
              _.speak(phrase.value, TextToSpeech.QUEUE_FLUSH, null)
            }
        })
  ...
  override def getView(position: Int, convertView: View, parent: ViewGroup): View = {
    ...
    //cache.get(position)
    speakButton(itemview, getItem(position))
    ...
    itemview.setOnLongClickListener(new View.OnLongClickListener {
      override def onLongClick(v: View): Boolean = {
        ...
        complimentary match {
          case None => {
            log("no comlimentary adpater")
          }
          case Some(x) => {
            log("do co,plimentary");
            x.add(item)
          }
        }
        true
  ...
  override def whole(viewgroup: ViewGroup): ViewGroup = {
    ...
    viewgroup.findViewById(R.id.word_pictures) match {
      ...
      case x: GridView => x.setAdapter(new ArrayAdapter(
        ...
        word.pictures.foldLeft(new java.util.ArrayList[Picture]())((array, x) => {
          array.add(x);
          array
        })) {
